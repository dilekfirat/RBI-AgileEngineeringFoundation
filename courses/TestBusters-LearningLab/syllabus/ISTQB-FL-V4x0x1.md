...
during the test analysis and design. Further information on test techniques can be found in the
...
(Watson 1996).
White-box test techniques can be used in static testing (e.g., during dry runs of code). They are well
suited to reviewing code not yet ready for execution (Hetzel 1988), pseudocode and other high-level or
...
Exploratory testing is sometimes performed using session-based testing to structure the testing. In a
session-based approach, exploratory testing is performed within a defined time box. The tester uses a
...
usually more accurate than for the large ones. Therefore, when estimating a large task, it can be
...
certain test levels than others. This model also provides a way to differentiate and describe the test types
...
or changed circumstances. Test completion reports summarize a specific test activity (e.g., test level, test
cycle, iteration) and can give information for subsequent testing.
During test monitoring and test control, the test team generates test progress reports for stakeholders to
...
the frequency of test reporting. Test progress reporting to others in the same team is often frequent and
...
typically comprises activities to log the reported anomalies, analyze and classify them, decide on a
suitable response such as to fix or keep it as it is and finally to close the defect report. The process must
be followed by all involved stakeholders. It is advisable to handle defects from static testing (especially
...
Pressman, R.S. (2019) Software Engineering. A Practitioner’s Approach, 9 th ed., McGraw Hill
...
3.2.2              Summarize the activities of the review process                                       K2                        X        X
                   Recall which responsibilities are assigned to the principal roles when performing    K1
3.2.3                                                                                                                                      X                                                                           X
                   reviews
...
Corrections in the text
      ...
      •      in 3.2.2 “a couple of times” replaced with “multiple times” because for large documents a couple of
             times is not enough
      •      in 4.2.1 “test object” replaced with “test item”, because this is the proper term in the context of
             applying test techniques
      •      in 4.2.1 added that invalid equivalence partitions should be tested in isolation to avoid defect
             masking
...
3-value BVA, 40                                                    control flow graph, 43
absence-of-defect fallacy, 18                                      coverage, 20, 40, 41, 42, 43, 45
acceptance criteria, 20, 45                                        coverage item, 19, 20, 40, 41, 42, 43, 44
acceptance test-driven development, 25, 26,                        coverage-based prioritization, 50
    46                                                             debugging, 16
acceptance testing, 29                                             decision table testing, 41
action item, 20                                                    defect, 17, 33, 34, 56
all states coverage, 42                                            defect management, 56
all transitions coverage, 42                                       defect report, 20, 36, 56
alpha testing, 29                                                  dependency (prioritization), 50
anomaly, 35, 56                                                    DevOps, 26, 56
author (reviews), 36                                               DevOps tool, 59
baseline, 56                                                       domain-driven design, 25
behavior-driven development, 25, 26                                driver, 20
beta testing, 29                                                   dynamic testing, 15, 34
black-box test technique, 39                                       Each Choice coverage, 40
black-box testing, 30                                              early testing, 18, 27, 33
boundary, 40                                                       entry criteria, 20, 49
boundary value analysis, 40                                        equivalence partitioning, 39
branch, 43                                                         error, 17
branch coverage, 43                                                error guessing, 44
branch testing, 43                                                 estimation, 49
burn-down chart, 55                                                estimation based on ratios, 49
business rule, 41                                                  executable requirement, 46
change request, 20                                                 executable statement, 42
checklist, 44                                                      exhaustive testing, 18
checklist-based testing, 44                                        exit criteria, 20, 37, 49
collaboration, 45                                                  experience-based test technique, 39, 43
collaboration tool, 59                                             exploratory testing, 44
collaboration-based test approach, 45                              extended-entry decision table, 41
communication, 55                                                  extrapolation, 49
compatibility, 30                                                  extreme programming, 25
component integration testing, 29                                  failure, 17, 34
component testing, 29                                              fault attack, 44
conditional branch, 43                                             feature-driven development, 25
configuration item, 56                                             fedback, 35
configuration management, 56                                       feedback, 37
confirmation bias, 22                                              flexibility, 30
confirmation testing, 16, 30                                       formal review, 36
containerization tool, 59                                          functional appropriateness, 29
continuous delivery, 26                                            functional completeness, 29
continuous improvement, 28                                         functional correctness, 29
continuous integration, 26                                         functional testing, 29

Given/When/Then, 26, 46                                            retrospective, 28
guard condition, 41                                                review, 33
harm, 52                                                           review leader, 36
hot fix, 31                                                        review process, 35
impact, 52                                                         review techniques, 33
impact analysis, 31                                                reviewer, 36
incremental development model, 25                                  risk, 15, 52, 55
independence of testing, 22                                        risk analysis, 52
independent test team, 29                                          risk assessment, 52
informal review, 36                                                risk control, 53
inspection, 37                                                     risk identification, 52
integration testing, 29                                            risk impact, 52
interaction capability, 30                                         risk level, 52
invalid partition, 40                                              risk likelihood, 52
INVEST, 45                                                         risk management, 51
iteration planning, 48                                             risk matrix, 53
iterative development model, 25                                    risk mitigation, 53
Kanban, 25                                                         risk monitoring, 53
Lean IT, 25                                                        risk register, 20
lessons learned, 20                                                risk-based prioritization, 50
likelihood, 52                                                     risk-based testing, 51
limited-entry decision table, 41                                   root cause, 17
maintainability, 30                                                safety, 30
maintenance testing, 31                                            scribe (reviews), 36
management tool, 59                                                Scrum, 25
manager (reviews), 36                                              SDLC, See software development lifecycle
metric, 54                                                         security, 30
mistake, See error                                                 sequential development model, 25
non-functional testing, 27, 29                                     service virtualization, 20
non-functional testing tool, 59                                    session-based testing, 44
operational acceptance testing, 29                                 shift-left, 27
pair testing, 19                                                   simulation, 29
Pareto principle, 18                                               simulator, 20
performance efficiency, 30                                         skill, 21
planning poker, 50                                                 software development lifecycle, 25
portability, 30                                                    specification, 30
prioritization, 50                                                 specification workshop, 46
product risk, 52                                                   spiral model, 25
project risk, 52                                                   state table, 41
prototyping, 25                                                    state transition diagram, 41
quality, 15, 16                                                    state transition testing, 41
quality assurance, 17                                              statement, 42
quality characteristic, 34                                         statement coverage, 42
quality control, 16, 17                                            statement testing, 42
regression testing, 16, 30                                         static analysis, 27, 33
release planning, 48                                               static testing, 15, 33, 43
reliability, 30                                                    static testing tool, 59
reporting, 54                                                      stub, 20
requirements-based prioritization, 50                              system integration testing, 29

system testing, 29                                                 test pyramid, 50
technical review, 37                                               test report, 54
test analysis, 19, 26                                              test result, 19, 56
test approach, 48                                                  test schedule, 20
test automation, 27, 59                                            test script, 19, 20
test automation framework, 46                                      test status, 55
test basis, 19, 20, 29                                             test strategy, 20, 48
test case, 19, 20, 50                                              test suite, 20, 50
test case prioritization, 50                                       test technique, 39
test charter, 20, 44                                               test tool, 59
test completion, 19, 54                                            test type, 29
test completion report, 20, 28, 55                                 testability, 19
test condition, 19, 20, 45, 46                                     test-driven development, 25, 26
test control, 19, 53                                               testing, 15, 16
test coverage tool, 59                                             testing quadrants, 51
test data, 19, 20                                                  testing role, 21
test design, 19, 26                                                testware, 19, 20
test design tool, 59                                               three-point estimation, 50
test effort, 49                                                    traceability, 20
test environment, 19, 20                                           transition, 41
test execution, 19                                                 unconditional branch, 43
test execution schedule, 19, 20                                    Unified Process, 25
test execution tool, 59                                            unit test framework, 29
test harness, 29                                                   usability, 30
test implementation, 19                                            user acceptance testing, 29
test implementation tool, 59                                       user story, 45
test level, 25, 28                                                 V model, 25
test log, 20                                                       valid partition, 40
test management role, 21                                           valid transitions coverage, 42
test metric, 54                                                    validation, 15, 33
test monitoring, 19, 53                                            verification, 15, 33
test object, 15, 19, 29                                            virtual machine, 59
test objective, 15, 25, 48                                         walkthrough, 36
test plan, 20, 48                                                  waterfall model, 25
test planning, 18, 48                                              white-box test technique, 39, 42
test policy, 48                                                    white-box testing, 30
